            ...
            <div class="columns is-centered has-text-centered">
                <div class="column is-half has-text-centered">
                   <form @submit.prevent="Update" class="card p-2 is-centered has-text-centered">
                       <b-image class="m-5 mb-5" :src="data.image" horizontal rounded>
                       </b-image>
                       <b-field label="Name"
                            horizontal>
...
export default {
    ...
    async created() {
        let resp = await this.$axios.get(`/api/top/${this.slug}`)
        this.data = resp.data
    },
    methods: {
        Update(event) {
            ...
            this.$axios.patch(`api/top/${this.slug}/`, formData, { headers: { "Content-Type": "multipart/form-data; boundary=<calculated when request is sent>" } })
            .then(response => {
