<template>
    <section class="container section">
        <br /> <br /> <br />
        <b-button @click="() => {this.$router.push('/admin/post/add')}" class="mb-2 is-success">
            add
        </b-button>
        ...
        <template>
            ...
            <b-table :data="data">
                ...
                <b-table-column field="slug" label="Slug">
                    <template v-slot="props">
                        <NuxtLink :to="`/admin/post/${props.row.id}`">
                            {{ props.row.title }}
                        </NuxtLink>
                    </template>
                </b-table-column>
                ...
                <b-table-column label="Actions" field="id" v-slot="props">
                    <div class="buttons">
                        ...
                        <b-button @click="$router.push(`/admin/post/${props.row.id}`)" class="is-success">
                             <b-icon
                                icon="pencil"
                                size="is-small"
                                type="is-white">
...
export default {
    ...
    methods: {
        ...
        Delete(slug) {
            this.loadin = true;
            this.$axios.delete(`api/post/${slug}/`)
            .then(() => {
                this.Refresh();
                this.loadin = false;
            })
