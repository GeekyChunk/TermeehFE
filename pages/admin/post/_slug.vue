                ...
                <div class="column is-half has-text-centered">
                   <form @submit.prevent="Update" class="card p-2 is-centered has-text-centered">
                        ...
                        <b-field label="Caption"
                            horizontal>
                            <textarea class="textarea" v-model="data.caption" maxlength="500"></textarea>
                        </b-field>
                        ...
                        <button @click.prevent="() => $router.push('/admin/post')" class="button is-danger">Discard</button>
...
export default {
    ...
    async created() {
        let resp = await this.$axios.get(`/api/post/${this.slug}`)
        this.data = resp.data
    },
    methods: {
        Update(event) {
            ...
            this.$axios.patch(`api/post/${this.slug}/`, formData, { headers: { "Content-Type": "multipart/form-data; boundary=<calculated when request is sent>" } })
            .then(response => {
